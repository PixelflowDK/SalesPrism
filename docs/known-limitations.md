...
| ~~2026-08-12~~ **CORRECTED same day (SD-012)** | Tenant is Entra ID Free — no licenses. The earlier claim that this blocks sign-in log export and break-glass alerting was **WRONG**, inferred from empty telemetry plus an unrelated P1 error. Microsoft's official licensing table lists **Sign-in logs and Audit logs as available on Entra ID Free**, and the Azure Monitor integration page states no licence prerequisite. AuditLogs rows are in fact arriving through that diagnostic setting. | **No purchase required for SR-008.** 9 of 10 criteria are achievable on Free. The tenth, Conditional Access exclusion, is conditional on CA being available; CA genuinely does require P1 (verbatim in Microsoft's licensing page), so no policy can exist that could block an emergency sign-in and there is nothing to exclude from — satisfied by non-applicability. | If P1 is ever purchased and CA enabled, both break-glass accounts must be excluded from the FIRST policy created. |
